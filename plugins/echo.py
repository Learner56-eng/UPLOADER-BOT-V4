...
async def echo(bot, update):
    ...
    if e_response and "nonnumeric port" not in e_response:
        ...
        if t_response:
            ...
            duration = response_json.get("duration", None)
            if "formats" in response_json:
                ...
                if "filesize" in response_json:
                    approx_file_size = humanbytes(response_json["filesize"])
            ...
            if Config.AUDIO_ENABLED:
                inline_keyboard.append([
                    InlineKeyboardButton(
                        "🎵 ᴜᴘʟᴏᴀᴅ ᴀᴜᴅɪᴏ", callback_data="audio_upload")
                ])
            if Config.VIDEO_ENABLED:
                inline_keyboard.append([
                    InlineKeyboardButton(
                        "🎬 ᴜᴘʟᴏᴀᴅ ᴠɪᴅᴇᴏ", callback_data="video_upload")
                ])
            if Config.DOCUMENT_ENABLED:
                inline_keyboard.append([
                    InlineKeyboardButton(
                        "📄 ᴜᴘʟᴏᴀᴅ ᴅᴏᴄᴜᴍᴇɴᴛ", callback_data="document_upload")
                ])

            await chk.delete()
            await bot.send_message(
                chat_id=update.chat.id,
                text=Translation.FORMAT_FOUND.format(str(file_name or "")),
                reply_to_message_id=update.id,
                parse_mode=enums.ParseMode.HTML,
                disable_web_page_preview=True,
